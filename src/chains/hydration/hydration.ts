...
/**
 * Main class for interacting with the Hydration blockchain.
 *
 * This class provides methods for account management, balance queries,
 * transaction operations, and network status for Hydration networks.
 */
export class HydrationChain {
  ...
  async getBalance(
    ...
    if (nativeToken) {
      const accountInfo = await this.apiPromiseQuerySystemAccount(
        apiPromise,
        address,
      );
    ...
    for (const token of tokensToCheck) {
      ...
      if (apiPromise.query.tokens && apiPromise.query.tokens.accounts) {
        const assetBalance = await this.apiPromiseQueryTokensAccounts(
          apiPromise,
          address,
          token.address,
        );
        ...
        if (assetBalance && !assetBalance.isEmpty) {
          ...
          const balance =
            ...
            '0';
          balances[token.symbol] = fromBaseUnits(balance, token.decimals);
        } else {
          balances[token.symbol] = 0;
        }
      } else {
  ...
  public async getTransaction(
    txHash: string,
    waitForFee: boolean = false,
    waitForTransfers: boolean = false,
  ): Promise<any> {
    ...
    try {
      ...
      while (
        Date.now() - startTime <
        1000 * Constant.defaultTimeout.getValueAs<number>()
      ) {
        try {
          ...
          if (response.data && response.data.data) {
            ...
            fee = transaction.fee
              ? parseFloat(transaction.fee) /
                Math.pow(10, feePaymentToken.decimals)
              : null;
        ...
        await sleep(
          1000 * Constant.defaultDelayBetweenRetries.getValueAs<number>(),
        );
      }
  ...
  async estimateTransactionGas(gasLimit?: number): Promise<any> {
    ...
    const fee = new BigNumber(paymentInfo.partialFee.toString()).div(
      new BigNumber(10).pow(feePaymentToken.decimals),
    );
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseQuerySystemAccount(
    target: ApiPromise,
    address: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseQueryAssetsAccount(
    target: ApiPromise,
    arg1: string,
    arg2: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseQueryTokensAccounts(
    target: ApiPromise,
    arg1: string,
    arg2: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseDeriveStakingAccount(
    target: ApiPromise,
    address: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseQueryStakingValidators(
    target: ApiPromise,
    address: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseRpcChainGetBlock(
    target: ApiPromise,
    blockHash: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseTxBalancesTransfer(
    target: ApiPromise,
    recipient: string,
    amount: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async apiPromiseTxBalancesTransferKeepAlive(
    target: ApiPromise,
    recipient: string,
    amount: string,
  ) {
  ...
  @runWithRetryAndTimeout()
  public async submittableExtrinsicPaymentInfo(
    target: any,
    sender: KeyringPair,
  ) {
