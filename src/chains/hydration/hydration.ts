...
import { getHydrationConfig } from './hydration.config';
import { createCipheriv, createDecipheriv, randomBytes } from 'crypto';
import { TokenListType, walletPath } from '../../services/base';
import { PollResponse } from './hydration.requests';
...
export class Hydration {
  ...
  private _chain: string = 'hydration';
  ...
  public controller: typeof hydrationController;
  ...
  constructor(
    ...
    const config = getHydrationConfig(network);
    this._network = network;
    this.nativeTokenSymbol = config.nativeCurrencySymbol;
    this.gasPrice = 0;
    this.nodeUrl = nodeUrl;
    this.hydrationApi = new ApiPromise({ provider: new WsProvider(nodeUrl) });
    this._keyring = new Keyring({ type: 'sr25519' });
    this._assetListType = assetListType;
    this._assetListSource = assetListSource;
    this.gasLimit = 0;
    this.gasCost = 0;
    this.controller = hydrationController;
  }
  public get hydration(): ApiPromise {
  ...
  public async init(): Promise<void> {
    ...
    this.hydrationApi = await ApiPromise.create({ provider });
  ...
  public static getInstance(network: string): Hydration {
    ...
    if (!Hydration._instances.has(config.network.name)) {
      if (network !== null) {
        ...
        Hydration._instances.set(
          config.network.name,
          new Hydration(network, nodeUrl, assetListType, assetListSource),
        );
      } else {
        throw new Error(
          `Hydration.getInstance received an unexpected network: ${network}.`,
        );
      }
    }
    return Hydration._instances.get(config.network.name) as Hydration;
  }

  public static getConnectedInstances(): { [name: string]: Hydration } {
    const connectedInstances: { [name: string]: Hydration } = {};
    if (this._instances !== undefined) {
      const keys = Array.from(this._instances.keys());
      for (const instance of keys) {
        if (instance !== undefined) {
          connectedInstances[instance as string] = this._instances.get(
            instance,
          ) as Hydration;
        }
      }
  ...
  public async getCurrentBlockNumber(): Promise<number> {
    const header = await this.hydrationApi.rpc.chain.getHeader();
  ...
  public async getAccountInfo(accountAddress: string): Promise<any> {
    const accountInfo =
      await this.hydrationApi.query.system.account(accountAddress);
  ...
  public async getTransaction(txHash: string): Promise<PollResponse> {
    const blockHash = await this.hydrationApi.rpc.chain.getBlockHash(txHash);
    const block = await this.hydrationApi.rpc.chain.getBlock(blockHash);
