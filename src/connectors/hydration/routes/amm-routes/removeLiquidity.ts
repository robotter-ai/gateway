...
export const removeLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  const hydration = await HydrationChain.getInstance('mainnet');
  let firstWalletAddress = '<hydration-wallet-address>';

  const foundWallet = await hydration.getFirstWalletAddress();
  if (foundWallet) {
