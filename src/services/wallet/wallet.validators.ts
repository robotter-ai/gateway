...
export const invalidHydrationChainPrivateKeyOrMnemonicError: string =
  'The privateKey param is not a valid Hydration private key or mnemonic.';
...
// TODO check!!!
export const isHydrationChainPrivateKeyOrMnemonic = (str: string): boolean => {
  const parts = str.split(' ');
...
export const validatePrivateKey: Validator = mkSelectingValidator(
  ...
  (req, key) => req[key],
  {
    ...
    'hydration-chain': mkValidator(
      'privateKey',
      invalidHydrationChainPrivateKeyOrMnemonicError,
      (val) => typeof val === 'string' && isHydrationChainPrivateKeyOrMnemonic(val),
    ),
  },
);
